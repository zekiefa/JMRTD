...
public class AESSecureMessagingWrapper extends SecureMessagingWrapper implements Serializable {
	...
	@Override
	public ResponseAPDU unwrap(final ResponseAPDU responseAPDU) {
		return this.unwrap(responseAPDU, responseAPDU.getData().length);
	}
	 ...
	 */
	private ResponseAPDU unwrap(ResponseAPDU responseAPDU, int len) {
		try {
